...
def firstIntegralMatrix(N, H):
  ...
  FIMat = diags([colm1, colp1], [-1, 1], shape = (N, N+2)).toarray()
  FIMat[0,N+1] = 1
  return H * FIMat
...
def secondIntegralMatrix(N, H):
  ...
  jj = np.arange(3, N)
  colm2_t = np.array([0.25])
  colp2_t = np.array([0, 0.125, 1. / 24.])
  col0_t = np.array([0, -0.125, -1. / 8. - 1. / 24.])
  ...
  SIMat = diags([colm2, col0, colp2], [-2, 0, 2], shape = (N,N+2)).toarray()
